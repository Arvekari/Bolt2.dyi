...
export const updateCustomPromptEnabled = (enabled: boolean) => {
  customPromptEnabledStore.set(enabled);
  localStorage.setItem(SETTINGS_KEYS.CUSTOM_PROMPT_ENABLED, JSON.stringify(enabled));
};
...
export const updateDbPostgresUrl = (url: string) => {
  dbPostgresUrlStore.set(url);
  localStorage.setItem('dbPostgresUrl', url);
};
