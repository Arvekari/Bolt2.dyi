...
import { resolveApiKeys, resolveProviderSettings } from '~/lib/api/cookies';
...
async function llmCallAction({ context, request }: ActionFunctionArgs) {
  ...
  const providerSettings = await resolveProviderSettings(cookieHeader, context.cloudflare?.env as any);
    ...
    try {
      ...
      const result = await streamText({
        ...
        messages: [
          {
            role: 'user',
            content: `${optimizedPrompt.messages[0]?.content || message}`,
          },
        ],
